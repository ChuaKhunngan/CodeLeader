...
deploy_watcher() {
  # Upload watcher from the skill bundle.
  if [ ! -f "$ROOT_DIR/assets/remote/prompt_watcher_v2.py" ]; then
    echo "Missing local watcher: $ROOT_DIR/assets/remote/prompt_watcher_v2.py" >&2
    exit 1
  fi
  cat "$ROOT_DIR/assets/remote/prompt_watcher_v2.py" | remote "cat > '$REMOTE_WATCHER' && chmod +x '$REMOTE_WATCHER'"
