...
2. OpenClaw 让执行继续留在那台远程机器上的 coding session 里，也就是项目和算力本来就所在的地方。
3. OpenClaw 会持续跟进这次执行，把工作拉回正轨，并处理日常审批。
4. 如果真的需要人类判断，OpenClaw 才会来问你。
5. 如果你想介入，可以通过 OpenClaw 发起，也可以直接进入远端 session 接手。
6. 之后工作会从当前真实状态继续推进。
...
- 你更希望围绕本地 GUI 环境工作，而不是围绕远端执行链路来组织开发
...
它让 **人类** 和 **OpenClaw** 始终围绕同一个真实工作面协作：
- 看到同一个真实状态
- 走同一条控制流程
- 一起决定什么时候让工作继续在远端 coding session 中执行

而实际执行，仍然发生在真实的远端 coding session 里。
