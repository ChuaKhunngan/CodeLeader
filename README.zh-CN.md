...
有了 CodeLeader，OpenClaw 做的不是“把任务交出去然后等结果”，而是持续对整体执行流程负责：

- 把工作送进真实的远程 coding session
- 不在启动后消失，而是持续跟进进度
- 当实现开始偏离轨道时，把工作拉回正轨
- 在可能的情况下自行处理日常审批与执行决策
- 只有在确实需要人类判断时，才把问题升级给人类
- 在人类需要了解进展时主动汇报状态
- 支持人类通过 OpenClaw 发起介入，或直接进入远端 coding session 接管
- 在介入结束后，从当前真实状态继续，而不是盲目恢复旧计划

一个最简单的使用场景是：

1. 你现在不在开发机旁边。
2. 你让 OpenClaw 去实现一个功能或修复一个 bug。
3. OpenClaw 把工作送进远端 coding session，并持续跟进这次执行。
4. 如果工作开始偏离轨道，OpenClaw 会把它拉回来。
5. 如果出现日常审批节点，OpenClaw 会先自行处理。
6. 如果真的出现需要人类判断的问题，OpenClaw 才会来问你。
7. 如果你愿意，你可以通过 OpenClaw 发起介入，或者直接进入远端 session 亲自接手。
8. 当介入结束后，工作会从当前真实状态继续推进。
